                ...
                <div className="flex-1 overflow-y-auto">
                    <form onSubmit={handleWithdraw} className="p-4 md:p-6 space-y-4 md:space-y-6">
                        ...
                        <div className="space-y-4">
                            <label className="text-center block text-sm text-slate-500 dark:text-[#cbbc90]">Enter Withdrawal Amount (USD)</label>
                            ...
                            <div className="flex justify-center">
                                <div className="inline-flex items-center space-x-2 bg-slate-100 dark:bg-[#1e1b10] px-3 py-1 rounded-full border border-slate-200 dark:border-[#f4c025]/20">
                                    <span className="text-xs text-slate-500 dark:text-[#cbbc90]">Max Withdrawable: <span className="text-slate-800 dark:text-white font-bold ml-1">${maxAmount.toLocaleString()}</span></span>
                                    <button type="button" onClick={handleMax} className="text-[10px] bg-amber-100 dark:bg-[#f4c025]/20 text-amber-600 dark:text-[#f4c025] px-2 py-0.5 rounded hover:bg-amber-200 dark:hover:bg-[#f4c025]/30 transition-colors">MAX</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex justify-between items-center bg-slate-100 dark:bg-[#f4c025]/5 p-3 rounded-lg border border-slate-200 dark:border-[#f4c025]/10">
                                <span className="text-xs text-slate-500 dark:text-[#cbbc90]">You will receive approx:</span>
                                <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400">Rp {estimatedIdr.toLocaleString('id-ID')}</span>
                            </div>
                        </div>
                        ...
                        <div className="bg-amber-50 dark:bg-[#f4c025]/5 border border-amber-200 dark:border-[#f4c025]/20 rounded-lg p-4 flex items-start space-x-3">
                            ...
                            <p className="text-xs text-slate-600 dark:text-[#cbbc90] leading-relaxed">
                                <span className="font-bold text-slate-800 dark:text-white">Accounting Notice:</span> Your Trading Wallet (USD) will be deducted, and the equivalent IDR value will be added to your Main Balance as 'Income'.
                            </p>
                        </div>
